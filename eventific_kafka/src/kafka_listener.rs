...
impl Listener for KafkaListener {
    ...
    fn listen<'a>(
        &'a self,
        logger: &'a Logger,
    ) -> BoxFuture<'a, Result<BoxStream<'a, Result<Uuid, NotificationError>>, NotificationError>>
    {
        async move {
            let consumer = self
                .consumer
                .as_ref()
                .expect("Listener has to be initialized");
            ...
            let mapped_stream = stream
                ...
                .and_then(|message| async move {
                    let payload =
                        message
                            .payload()
                            .ok_or(NotificationError::FailedToListen(format_err!(
                                "Missing payload"
                            )))?;
                    let raw_uuid = from_utf8(payload).map_err(|_| {
                        NotificationError::FailedToListen(format_err!("Invalid payload encoding"))
                    })?;
                    let aggregate_id = Uuid::parse_str(raw_uuid).map_err(|_| {
                        NotificationError::FailedToListen(format_err!(
                            "The payload was not a valid Uuid"
                        ))
                    })?;
                    Ok(aggregate_id)
                });
