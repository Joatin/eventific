...
pub(crate) trait IntoEvent<D: EventData, M = ()> {
    fn into_event(self, aggregate_id: Uuid, base_event_id: u32, metadata: Option<M>) -> Vec<Event<D, M>>;
}

impl<D: EventData, M: Clone> IntoEvent<D, M> for Vec<D> {
    fn into_event(self, aggregate_id: Uuid, mut base_event_id: u32, metadata: Option<M>) -> Vec<Event<D, M>> {
        self.into_iter()
            .map(|data| {
                let e = Event {
                    aggregate_id,
