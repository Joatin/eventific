...
impl SqsListener {
    ...
    fn handle_message(
        logger: Logger,
        result: Result<ReceiveMessageResult, RusotoError<ReceiveMessageError>>,
        client: SqsClient,
        queue_url: String,
    ) -> impl Future<Item = Vec<Uuid>, Error = NotificationError> {
        match result {
            Ok(message) => Either::A(Self::handle_message_result(
                &logger, message, client, queue_url,
            )),
        ...
        match body {
            Some(u) => match Uuid::parse_str(&u) {
                ...
                Err(_) => {
                    warn!(
                        logger,
                        "Received an invalid uuid, the invalid id was: {}", u
                    );
                    Uuid::default()
                }
            },
    ...
    fn handle_message_error(
        logger: &Logger,
        error: RusotoError<ReceiveMessageError>,
    ) -> impl Future<Item = Vec<Uuid>, Error = NotificationError> {
        match error {
            RusotoError::HttpDispatch(err) => {
                ...
                Either::A(
                    Delay::new(Instant::now().add(Duration::from_millis(1000)))
                        .map_err(|d_err| {
                            NotificationError::FailedToListen(format_err!("{}", d_err))
                        })
                        .and_then(|_| Ok(vec![])),
                )
            }
            _ => Either::B(futures::failed(NotificationError::FailedToListen(
                format_err!("{}", error),
            ))),
        }
    }
...
impl Listener for SqsListener {
    ...
    fn listen(&self) -> Box<dyn Stream<Item = Uuid, Error = NotificationError> + Send> {
        ...
        let logger = self
            .logger
            .clone()
            .expect("The listener needs to be initialized");
