...
impl<D> PostgresStore<D> {
    ...
    async fn create_table(logger: &Logger, client: &Client, service_name: &str) -> Result<(), tokio_postgres::Error> {
        client
            .simple_query(&format!(
...
impl<D: EventData + Serialize + DeserializeOwned, M: 'static + Send + Sync + Debug + Serialize + DeserializeOwned> Store<D, M> for PostgresStore<D> {
    ...
    fn events<'a>(
        ...
        async move {
            ...
            let row_stream = client.query_raw(&statement, params.iter().map(|p| p as &dyn ToSql))
                .await
                .map_err(|err| StoreError::Unknown(format_err!("{:?}", err)))?;
    ...
    fn total_aggregates<'a>(
        ...
        async move {
            ...
            let statement = client.prepare(&format!("SELECT COUNT(DISTINCT aggregate_id) FROM {}_event_store", service_name))
                .await
                .map_err(|err| StoreError::Unknown(format_err!("{:?}", err)))?;
    ...
    fn total_events_for_aggregate<'a>(
        ...
        async move {
            ...
            let statement = client.prepare(&format!("SELECT COUNT(*) FROM {}_event_store WHERE aggregate_id = $1", service_name))
                .await
                .map_err(|err| StoreError::Unknown(format_err!("{:?}", err)))?;
    ...
    fn total_events<'a>(
        ...
        async move {
            ...
            let statement = client.prepare(&format!("SELECT COUNT(*) FROM {}_event_store", service_name))
                .await
                .map_err(|err| StoreError::Unknown(format_err!("{:?}", err)))?;
