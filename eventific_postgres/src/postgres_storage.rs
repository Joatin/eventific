...
use eventific::{SaveEventsResult, Storage};
use futures::stream;
    ...
    <<Tls as MakeTlsConnect<Socket>>::TlsConnect as TlsConnect<Socket>>::Future: Send,
{
                        ...
                        .map(|row_res| match row_res {
                            Ok(row) => {
                                ...
                                match serde_json::from_value::<P>(raw_payload) {
                                    Ok(payload) => Ok((event_id, payload)),
    ...
    async fn total_events_for_aggregate(&self, aggregate_id: &uuid::Uuid) -> anyhow::Result<u64> {
        ...
        let row_list = client
            .query(
                &format!(
                    "SELECT COUNT(*) FROM {} WHERE aggregate_id='{}'",
                    self.table_name, aggregate_id
                ),
                &[],
            )
            .await?;
    ...
    async fn save_events<'a>(
        ...
        aggregate_id: &uuid::Uuid,
        events: Vec<(u64, P)>,
    ) -> eventific::SaveEventsResult
    where
        P: 'a,
    {
        match self.pool.get().await {
            Ok(mut client) => match client.transaction().await {
                Ok(transaction) => {
                    match transaction
                        .prepare(&format!(
                                ...
                                match serde_json::to_value(event.1) {
                                    Ok(payload) => {
                                        if let Err(err) = transaction
                                            .execute(
                                                &statement,
                                                &[aggregate_id, &(event.0 as i64), &payload],
                                            )
                                            .await
                                        {
                                    ...
                                    Err(err) => return SaveEventsResult::Error(err.into()),
                                }
                            }
    ...
    async fn aggregate_version(&self, aggregate_id: &uuid::Uuid) -> anyhow::Result<u64> {
        ...
        let row_list = client
            .query(
                &format!(
                    "SELECT event_id FROM {} WHERE aggregate_id = $1 ORDER BY event_id DESC;",
                    self.table_name
                ),
                &[aggregate_id],
            )
    ...
    async fn all_aggregate_ids(
        &self,
    ) -> futures::stream::BoxStream<'_, anyhow::Result<uuid::Uuid>> {
        match self.pool.get().await {
            Ok(client) => {
                match client
                    .query_raw::<_, _, &Vec<String>>(
                        format!("SELECT DISTINCT aggregate_id FROM {}", self.table_name).as_str(),
                        &vec![],
                    )
                    .await
                {
