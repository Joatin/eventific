import { assert, object, array, any } from 'joi';
import { InternalLogger, Logger, Store, Bootstrapable } from '@eventific/core';
import chalk from 'chalk';
import { get } from 'node-emoji';

var queryManagerOptionsSchema = object().keys({
    aggregates: array().min(1).required(),
    providers: array().optional(),
    store: any().required(),
    viewHandlers: array().min(1).required()
});
...
function QueryManager(options) {
    assert(options, queryManagerOptionsSchema);
    ...
    return function (Class) {
        return _a = /** @class */ (function (_super) {
                ...
                class_1._InstantiateViewHandler = function (parentInjector) {
                    ...
                    injector.set({
                        provide: Logger,
                        useConstant: new InternalLogger(chalk.blue(pascalCase$1(options.name) + "CommandHandler"))
                    });
...
export { QueryManager, IQueryManager, ViewHandler, IViewHandler };
